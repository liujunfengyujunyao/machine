...
class TixianController extends Controller{
   ...
   public function actionAct_tixian()//第一步
   {
      ...
      if($result->return_code=='SUCCESS' && $result->result_code=='SUCCESS') {
                ...
                if($res){
                    ...
                    $res4 = D('member')->where(['userid'=>$user_id])->setDec('balance',$this->amount);//可提现的最大额
                }else{
                    $res3 = $cash->add($data);
                }
   ...
   public function tiXianAction(){
      ...
      //证书地址,微信支付下面
        ...
        curl_setopt($ch,CURLOPT_SSLCERT, 'D:\workspace\machine\ThinkPHP\Library\Vendor\Weixinpay\certapiclient_cert.pem'); //证书这块大家把文件放到哪都行、
        curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
        curl_setopt($ch,CURLOPT_SSLKEY,  'D:\workspace\machine\ThinkPHP\Library\Vendor\Weixinpay\certapiclient_key.pem');//注意证书名字千万别写错、
