    ...
    **/
    ...
    public function getParameters(){
        ...
        if (!isset($_GET['code'])) {
            ...
            // 返回的url
            // $redirect_uri=U('http://liujunfeng.imwork.net:41413/Home/Weixinpay/pay','','',true);dump($redirect_uri);die;
            $redirect_uri = "http://liujunfeng.imwork.net:41413/Home/Weixinpay/pay";dump($redirect_uri);die;
            $redirect_uri=urlencode($redirect_uri);dump($redirect_uri);die;
            $url='https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$config['APPID'].'&redirect_uri='.$redirect_uri.'&response_type=code&scope=snsapi_base&state='.$out_trade_no.'#wechat_redirect';dump($url);
            redirect($url);die;
        }else{
          dump("yes");die;
