...
class EquipmentModel extends Model{
	...
	protected $_auto=array(
		//商品添加时间自动完成
		array('equipment_create_time','time',1,'function')
	);
	...
	//多文件上传
	public function upload_pics($equipment_id,$files){
		//判断是否有文件上传
		if (!isset($files['equipment_pics'])||min($files['equipment_pics']['error']) !=0) {
			return false;
		}
		...
		foreach ($res_upload as $k => $v) {
			...
			D('Equipmentpics')->add($row);
		}


			//实例化 Equipmentspics 模型类 关联 equipmentspics 数据表 完成添加数据
